    ...
    public static void warnIfDeploymentDirectoryIsInsecure() {
        try {
            ...
            if (isWritableByUnsafePrincipal(acl)) {
                LOGGER.Error("The program directory {dir} grants write access to unprivileged users, allowing a lower-privileged user to plant a malicious DLL (DLL search-order hijacking) or overwrite priority.txt. Move the executable to a protected directory such as C:\\Program Files\\AuthenticatorChooser.", path);
            }
        } catch (Exception e) {
